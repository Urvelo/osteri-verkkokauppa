...
(function() {
  ...
  function renderProducts() {
    ...
    for (var _i = 0; _i < toShow.length; _i++) {
      ...
      // Build meta line based on visibility settings
      var metaHtml = '';
      if (p.showRating) metaHtml += '<span class="stars">' + renderStars(p.score) + '</span>';
      if (p.showSales) metaHtml += '<span>' + formatOrders(p.orders) + ' myyty</span>';

      // Build price row
      var priceRowHtml = '<span class="product-price">' + formatPrice(sp) + '</span>';
      if (p.showOriginalPrice && op > sp) priceRowHtml += '<span class="product-orig-price">' + formatPrice(op) + '</span>';
      if (p.showDiscount && p.discount) priceRowHtml += '<span class="product-discount">' + esc(p.discount) + '</span>';

      html += '<div class="product-card" onclick="openProduct(\'' + escAttr(p.id) + '\')">' +
        '<div class="product-image-wrap">' +
        '<img src="' + escAttr(p.image) + '" alt="' + escAttr(pTitle(p)) + '" class="product-image" loading="lazy" referrerpolicy="no-referrer" ' +
        ...
        '<h3 class="product-title">' + esc(pTitle(p)) + '</h3>' +
        (metaHtml ? '<div class="product-meta">' + metaHtml + '</div>' : '') +
        '<div class="product-price-row">' + priceRowHtml +
        '</div></div>' +
