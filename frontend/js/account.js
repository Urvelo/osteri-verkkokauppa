...
(function() {
  ...
  function renderOrderHistory(orders) {
    ...
    orders.forEach(function(order) {
      ...
      (order.items || []).forEach(function(item) {
        var linkStart = item.productId ? '<a onclick="openProduct(\'' + escAttr(item.productId) + '\')" style="color:var(--accent);cursor:pointer">' : '';
        var linkEnd = item.productId ? '</a>' : '';
        var imgClick = item.productId ? 'openProduct(\'' + escAttr(item.productId) + '\')' : '';
        itemsHtml += '<div class="order-item-row">' +
          '<img src="' + escAttr(item.image || '') + '" alt="" onerror="this.style.display=\'none\'" onclick="' + imgClick + '">' +
          '<div class="item-title">' + linkStart + esc(item.title) + linkEnd +
          (item.variant ? '<br><small style="color:var(--fg-muted)">' + esc(item.variant) + '</small>' : '') + '</div>' +
      ...
      html += '<div class="order-card" id="order_' + order.id + '">' +
        ...
        esc(c.address || '') + '<br>' + esc(c.postal || '') + ' ' + esc(c.city || '') + ', ' + esc(c.country || '') + '</div>' +
  ...
  document.addEventListener('DOMContentLoaded', function() {
    (typeof dataReady !== 'undefined' ? dataReady : Promise.resolve()).then(initAccount);
  });
})();
