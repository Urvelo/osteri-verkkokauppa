...
function zoomImage(src) {
  ...
  // Reset & load: no-referrer bypasses CDN hotlink checks
...
function buildReceiptHTML(order) {
  ...
  (order.items || []).forEach(function(item) {
    var clickAttr = item.productId ? ' onclick="closeReceipt();openProduct(\'' + escAttr(item.productId) + '\')" style="color:#1a68b0;cursor:pointer;text-decoration:underline"' : '';
    itemRows += '<tr><td' + clickAttr + '>' + esc(item.title) +
      (item.variant ? '<br><small style="color:#999">' + esc(item.variant) + '</small>' : '') +
  ...
  var html = '<div class="receipt-header"><h1>ERÄT.FI</h1><p>Kuitti / Tilausvahvistus</p></div>' +
    ...
    '<div><b>Asiakas:</b> ' + esc(c.firstName || '') + ' ' + esc(c.lastName || '') +
    '<br><b>S\u00e4hk\u00f6posti:</b> ' + esc(c.email || '') + '<br><b>Puhelin:</b> ' + esc(c.phone || '') + '</div></div>' +
    '<div style="margin-bottom:8px;font-size:.85rem"><b>Toimitusosoite:</b> ' +
    esc(c.address || '') + ', ' + esc(c.postal || '') + ' ' + esc(c.city || '') + ', ' + esc(c.country || '') + '</div>' +
...
async function injectSharedUI() {
  ...
  // Zoom overlay
  var zoomHTML = '<div class="zoom-overlay" id="zoomOverlay" onclick="this.classList.remove(\'open\')"><img id="zoomImg" referrerpolicy="no-referrer"></div>';
  ...
  var footerHTML = '<footer><div class="container">' +
    ...
    '<a href="https://www.instagram.com/erat.fi/" target="_blank" rel="noopener" title="Instagram" style="color:var(--fg-muted);text-decoration:none;transition:color .15s;"><svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M7.8 2h8.4C19.4 2 22 4.6 22 7.8v8.4a5.8 5.8 0 0 1-5.8 5.8H7.8C4.6 22 2 19.4 2 16.2V7.8A5.8 5.8 0 0 1 7.8 2m-.2 2A3.6 3.6 0 0 0 4 7.6v8.8C4 18.39 5.61 20 7.6 20h8.8a3.6 3.6 0 0 0 3.6-3.6V7.6C20 5.61 18.39 4 16.4 4H7.6m9.65 1.5a1.25 1.25 0 1 1 0 2.5 1.25 1.25 0 0 1 0-2.5M12 7a5 5 0 1 1 0 10 5 5 0 0 1 0-10m0 2a3 3 0 1 0 0 6 3 3 0 0 0 0-6z"/></svg></a>' +
    
    '<a href="https://www.youtube.com/@Er%C3%A4t-fi" target="_blank" rel="noopener" title="YouTube" style="color:var(--fg-muted);text-decoration:none;transition:color .15s;"><svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg></a>' +
    '</div>' +
    '<div style="max-width: 800px; margin: 30px auto 10px;">        <h3 style="font-size: 1.1rem; margin-bottom: 20px; color: #ccc; font-weight: 500;">Hyväksytyt maksutavat</h3>        <div style="display: flex; flex-wrap: wrap; gap: 15px; justify-content: center; align-items: center;">          <div style="background: #fff; padding: 8px 14px; border-radius: 6px; display: flex; align-items: center; justify-content: center;"><img src="https://upload.wikimedia.org/wikipedia/commons/f/f2/Google_Pay_Logo.svg" alt="Google Pay" style="height: 18px; object-fit: contain; display: block;" /></div>                    <div style="background: #fff; padding: 8px 14px; border-radius: 6px; display: flex; align-items: center; justify-content: center;"><img src="https://upload.wikimedia.org/wikipedia/commons/c/c5/MobilePay_logo.svg" alt="MobilePay" style="height: 18px; object-fit: contain; display: block;" /></div>          <div style="background: #fff; padding: 8px 14px; border-radius: 6px; display: flex; align-items: center; justify-content: center;"><img src="https://upload.wikimedia.org/wikipedia/commons/b/b5/PayPal.svg" alt="PayPal" style="height: 18px; object-fit: contain; display: block;" /></div>          <div style="background: #fff; padding: 8px 14px; border-radius: 6px; display: flex; align-items: center; justify-content: center;"><img src="https://upload.wikimedia.org/wikipedia/commons/5/5e/Visa_Inc._logo.svg" alt="Visa" style="height: 14px; object-fit: contain; display: block;" /></div>          <div style="background: #fff; padding: 8px 14px; border-radius: 6px; display: flex; align-items: center; justify-content: center;"><img src="https://upload.wikimedia.org/wikipedia/commons/2/2a/Mastercard-logo.svg" alt="Mastercard" style="height: 18px; object-fit: contain; display: block;" /></div>        </div>      </div>' +
      '<p style="margin-top:30px;font-size:.7rem;color:#444;">' +
    '<a href="' + ROOT + 'palautus/" style="color:#555">Palautuslomake</a> \u2022 ' +
    '<a href="' + ROOT + 'tiedot/tietoja.html" style="color:#555">Tietoja</a></p>' +
    '<p style="margin-top:8px;font-size:.7rem;color:#444;">Myyjä: Tobias Humalajoki (yksityishenkilö) &ndash; Suomi</p>' +
    '<p style="margin-top:4px;font-size:.7rem;color:#444;">\u00a9 2026 Erät.fi \u2013 Kaikki oikeudet pid\u00e4tet\u00e4\u00e4n.</p>' +
    '</div></footer>';
...
function trackVisit() {
  ...
  var isNewVisitor = !localStorage.getItem('rk_visit_' + today);
  if (isNewVisitor) {
    localStorage.setItem('rk_visit_' + today, '1');
    // Clean old flags (keep only last 3 days)
    for (var i = localStorage.length - 1; i >= 0; i--) {
      var k = localStorage.key(i);
      if (k && k.startsWith('rk_visit_') && k !== 'rk_visit_' + today) {
        localStorage.removeItem(k);
      }
    }
  }

  // Get country from free IP geolocation API (once per visitor per day)
  if (isNewVisitor) {
    fetch('https://api.country.is/')
      .then(function(r) { return r.json(); })
      .then(function(data) {
        var cc = data.country;
        var country = /^[A-Z]{2}$/.test((cc || '').trim()) ? cc.trim() : 'XX';
        saveVisit(today, country, isNewVisitor);
      })
      .catch(function() { saveVisit(today, 'XX', isNewVisitor); });
  } else {
    saveVisit(today, null, false);
  }
}

function saveVisit(date, countryCode, isNewVisitor) {
  if (typeof db === 'undefined') return;
  var device = /Mobi|Android|iPhone|iPad/i.test(navigator.userAgent) ? 'mobile' : 'desktop';
  var page = (typeof PAGE_ID !== 'undefined' && PAGE_ID) ? PAGE_ID : (location.pathname.replace(/\//g, '_').replace(/^_|_$/g, '') || 'home');
  var FV = firebase.firestore.FieldValue;
  var update = {
    date: date,
    pageviews: FV.increment(1),
    ['devices.' + device]: FV.increment(1),
    ['topPages.' + page]: FV.increment(1)
  };
  if (isNewVisitor) {
    update.visitors = FV.increment(1);
    update.sessions = FV.increment(1);
    if (countryCode) update['countries.' + countryCode] = FV.increment(1);
  }
  db.collection('analytics').doc(date).set(update, { merge: true })
    .catch(function(e) { console.warn('Analytics write error:', e); });
}
